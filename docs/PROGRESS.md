# گزارش پیشرفت پروژه Doc-Gen-System

> این سند، مکمل `README.md` اصلی است و به‌صورت گام‌به‌گام مسیری که تا این لحظه طی شده را مستند می‌کند: چه کاری، چرا، و چطور انجام شد. برای راه‌اندازی و معماری کلی پروژه به `README.md` مراجعه کنید.

---

## فهرست

1. [نمای کلی مسیر](#نمای-کلی-مسیر)
2. [فاز ۱ — آپلود سند Device](#فاز-۱--آپلود-سند-device)
3. [چالش‌ها و مشکلات کلیدی پروژه](#چالش‌ها-و-مشکلات-کلیدی-پروژه)
4. [سخت‌سازی امنیتی آپلود](#سخت‌سازی-امنیتی-آپلود)
5. [پشتیبانی چندگانه از LLM (شامل OpenRouter)](#پشتیبانی-چندگانه-از-llm-شامل-openrouter)
6. [فاز ۲.۵ (بخش ۱) — پارسر ساختاری Device DOCX](#فاز-۲۵-بخش-۱--پارسر-ساختاری-device-docx)
7. [ساختار برنچ‌ها](#ساختار-برنچ‌ها)
8. [نقشه‌راه باقی‌مانده](#نقشه‌راه-باقی‌مانده)

---

## نمای کلی مسیر

```mermaid
flowchart TD
    A["فاز ۱<br/>Upload Device DOCX"] --> B["رفع باگ‌های زیرساخت<br/>Migration + Docker"]
    B --> C["سخت‌سازی امنیتی<br/>حجم / نوع فایل / نام فایل"]
    C --> D["پشتیبانی چند LLM<br/>OpenRouter و غیره"]
    D --> E["فاز ۲.۵ (بخش ۱)<br/>پارسر Device DOCX"]
    E --> F["فاز ۲.۵ (بخش ۲)<br/>خط‌لوله‌ی Vision<br/>(در پیش رو)"]
    F --> G["فاز ۳.۵<br/>Device در Knowledge Base"]
    G --> H["فاز ۴<br/>Agent مقایسه‌گر"]
    H --> I["فاز ۵<br/>DOCX Builder واقعی"]

    style A fill:#d5e8f0
    style B fill:#d5e8f0
    style C fill:#d5e8f0
    style D fill:#d5e8f0
    style E fill:#d5e8f0
    style F fill:#fff2cc
    style G fill:#f5f5f5
    style H fill:#f5f5f5
    style I fill:#f5f5f5
```

بخش‌های آبی‌رنگ **انجام‌شده**، زرد **در پیش رو (قدم بعدی)**، و خاکستری **برنامه‌ریزی‌شده برای آینده** هستند.

---

## فاز ۱ — آپلود سند Device

**هدف:** امکان آپلود یک فایل سند (Word) و اتصال آن به یک Device، دقیقاً مشابه جریان آپلود Reference که از قبل وجود داشت.

**چه چیزی ساخته شد:**
- دکمه‌ی «Upload Device» در صفحه‌ی Devices
- Endpoint بک‌اند: `POST /devices/documents`
- مدل دیتابیس `DeviceDocument` و جدول `device_documents`
- اگر نام فایل با نام یک Device موجود مطابقت نداشته باشد، یک Device جدید به‌صورت خودکار ساخته می‌شود

```mermaid
sequenceDiagram
    participant U as کاربر
    participant FE as Frontend
    participant BE as Backend
    participant DB as PostgreSQL
    participant FS as فایل‌سیستم

    U->>FE: انتخاب فایل و کلیک Upload
    FE->>BE: POST /devices/documents
    BE->>FS: ذخیره‌ی فایل در device_documents/
    BE->>DB: ثبت ردیف در device_documents
    DB-->>BE: تأیید
    BE-->>FE: 200 OK
    FE-->>U: نمایش «Upload successful»
```

---

## چالش‌ها و مشکلات کلیدی پروژه

در طول این مسیر، چند دسته مشکل واقعی و قابل‌تکرار شناسایی و رفع شد. این بخش آن‌ها را در سطح ریشه‌ای (نه جزئیات دستورهای اجراشده) دسته‌بندی می‌کند، چون شناخت الگوی کلی مشکل برای جلوگیری از تکرارش در آینده مفیدتر از مرور قدم‌به‌قدم است.

### ۱. دسترسی و مجوز روی GitHub

Push کردن تغییرات به ریپازیتوری با چند بار تلاش ناموفق مواجه شد؛ علت هر بار متفاوت بود:

- توکنی که ساخته شده بود، **اصلاً معتبر نبود** (۴۰۱ در برابر API گیت‌هاب)
- توکن بعدی معتبر بود ولی **دسترسی نوشتن (Write) روی مخزن نداشت** (۴۰۳ Permission denied)
- ریشه‌ی مشترک: هنگام ساخت Fine-grained Personal Access Token، دسترسی `Contents: Read and write` به‌صورت صریح روی مخزن هدف انتخاب نشده بود.

**نتیجه‌گیری:** برای این نوع کار، ساده‌ترین و امن‌ترین مسیر، push کردن مستقیم از طریق ترمینال شخصی کاربر (با احراز هویت خودش در VS Code) بود؛ نه انتقال توکن در چت.

### ۲. ناهماهنگی نسخه‌ی دیتابیس (Migration Drift)

دیتابیس پروژه در دو نوبت با تاریخچه‌ی migration فایل‌های فعلی هم‌خوانی نداشت:

- جدول کنترل نسخه (`alembic_version`) به یک شناسه‌ی migration اشاره می‌کرد که در فایل‌های موجود پروژه اصلاً وجود نداشت (احتمالاً باقی‌مانده از یک نسخه‌ی متفاوت یا اجرای دستی قبلی)
- حتی پس از هم‌تراز کردن نسخه، خود جدول `device_documents` از قبل با **ستون‌های متفاوت** از چیزی که مدل فعلی انتظار داشت وجود داشت

**نتیجه‌گیری:** ناهماهنگی migration معمولاً یک علامت است، نه یک مشکل مجزا — نشانه‌ی این‌که دیتابیس در جایی خارج از مسیر استاندارد Alembic دستکاری شده. راه‌حل پایدار، هم‌تراز کردن صریح `alembic_version` و در صورت نیاز، بازسازی کامل جدول‌های ناسازگار بود، نه patch زدن نقطه‌ای.

### ۳. عدم بازتاب کد جدید در محیط اجرا (Docker Build Cache)

چندین بار، تغییرات کد (چه در Backend چه در Frontend) پس از اعمال، در رفتار واقعی سیستم دیده نمی‌شد. علت این بود که Docker یک **ایمیج از پیش ساخته‌شده و کش‌شده** را بدون بازسازی مجدد اجرا می‌کرد.

**نتیجه‌گیری:** `docker compose up -d` به‌تنهایی برای اعمال تغییرات کد کافی نیست؛ باید همراه با `--build --force-recreate` روی همان سرویس اجرا شود. این یک قاعده‌ی ثابت برای ادامه‌ی کار روی این پروژه است.

### ۴. ناپایداری خود Docker Engine

در یک مقطع، Docker Desktop عملاً از پاسخ‌گویی بازماند (خطای «Bad response from Docker engine») و ایمیج‌های ساخته‌شده موقتاً از دسترس خارج شدند. علت محتمل، پر شدن فضای دیسک از انباشت ایمیج‌های میانی بی‌استفاده بود.

**نتیجه‌گیری:** پاک‌سازی دوره‌ای و امن ایمیج‌های بلااستفاده (`docker image prune`، نه `system prune -a` که به‌طور کامل cache معتبر را هم پاک می‌کند) باید به یک عادت نگهداری تبدیل شود.

### ۵. نبود محدودیت در آپلود فایل (ریسک امنیتی)

نسخه‌ی اولیه‌ی قابلیت آپلود سند Device هیچ سقفی برای حجم فایل، هیچ محدودیتی برای نوع فایل، و هیچ پاک‌سازی‌ای برای نام فایل نداشت — ترکیبی که هم امکان حمله‌ی انکار سرویس (پر کردن دیسک) و هم دسترسی خارج از مسیر مجاز (Path Traversal) را باز می‌گذاشت. این مورد به‌صورت مجزا شناسایی و در برنچ اختصاصی رفع شد (جزئیات در بخش «سخت‌سازی امنیتی» در ادامه).

### ۶. رفتار غیرمنتظره‌ی مدل‌های LLM «Reasoning»

هنگام افزودن پشتیبانی از OpenRouter، برخی مدل‌های نوع reasoning (مانند نسخه‌ای که در آن مقطع پشت نام مستعار «Ox Alpha» در دسترس بود) در برخی درخواست‌ها فیلد محتوای اصلی پاسخ را **خالی** برمی‌گرداندند، چون بخش عمده‌ی خروجی مدل صرف مرحله‌ی استدلال داخلی می‌شد. کد موجود فرض کرده بود این فیلد همیشه مقدار دارد.

**نتیجه‌گیری:** برای تولید سند نهایی (که نیاز به خروجی متنی قطعی دارد)، مدل‌های استاندارد (غیر-reasoning) پایدارتر عمل کردند. مدیریت صریح حالت «پاسخ خالی» در لایه‌ی LLM، مورد آینده‌ای است که هنوز به‌طور کامل رفع نشده.

### ۷. عدم اعتبارسنجی داده‌ی استخراج‌شده در پارسر جدید

پارسر تازه‌ساخته‌شده برای Device DOCX (فاز ۲.۵)، بدون بررسی، عناوین استخراج‌شده را مستقیم در ستونی با محدودیت طول ۲۵۵ کاراکتر درج می‌کرد. یک عنوان طولانی‌تر از حد مجاز (که در عمل معمولاً نشانه‌ی تشخیص اشتباه یک پاراگراف به‌جای heading است) کل درخواست آپلود را با خطای دیتابیس متوقف می‌کرد.

**نتیجه‌گیری:** هر مسیر استخراج خودکار از یک منبع خارجی (فایل کاربر) باید ورودی را نسبت به محدودیت‌های ذخیره‌سازی مقاوم فرض کند، نه معتبر؛ این اصل اکنون در پارسر اعمال شده است.

### ۸. ناسازگاری نحوی نمودارهای Mermaid با موتور رندر GitHub

نمودارهای مستندسازی (همین سند) در ویرایش اول به‌صورت متن خام روی GitHub نمایش داده شدند، نه به‌صورت گرافیکی. علت، چند ناسازگاری نحوی مشخص بود (کاراکتر شکست خط نامعتبر در برچسب‌ها، برچسب یال حاوی یک رشته‌ی شبه-پرچم خط‌فرمان، و نام برنچ حاوی `/` در نمودار gitGraph) که موتور Mermoid گیت‌هاب نسبت به آن‌ها سخت‌گیرتر از پیش‌نمایش‌های محلی است.

**نتیجه‌گیری:** نمودارهای Mermaid پیش از commit باید با ابزار تجزیه‌گر واقعی اعتبارسنجی شوند، نه فقط با چشم بازبینی شوند.

---

## سخت‌سازی امنیتی آپلود

نسخه‌ی اولیه‌ی آپلود Device هیچ محدودیتی نداشت (حجم نامحدود، هر نوع فایل، نام فایل خام). این ریسک‌های زیر شناسایی و برطرف شدند:

```mermaid
flowchart TD
    subgraph before["قبل"]
        A1["حجم فایل: نامحدود"]
        A2["نوع فایل: هر چیزی"]
        A3["نام فایل: مستقیم از کاربر<br/>(ریسک Path Traversal)"]
    end
    subgraph after["بعد"]
        B1["حجم فایل: سقف ۵۰ مگابایت<br/>(چک‌شده حین استریم، نه بعد از آپلود کامل)"]
        B2["نوع فایل: فقط docx. / doc. / pdf."]
        B3["نام فایل: sanitize می‌شود<br/>(حذف مسیر و کاراکترهای غیرمجاز)"]
    end
    before -.->|"اصلاح شد"| after

    style A1 fill:#fce4e4
    style A2 fill:#fce4e4
    style A3 fill:#fce4e4
    style B1 fill:#e4f7e4
    style B2 fill:#e4f7e4
    style B3 fill:#e4f7e4
```

**تغییرات فنی:**
- `settings.max_device_document_upload_bytes` (پیش‌فرض: ۵۰ مگابایت) در `config.py`
- بررسی حجم به‌صورت **استریم** (نه بعد از آپلود کامل) در `device_document_service.py` — یعنی یک فایل حجیم عمداً‌ فرستاده‌شده، همان لحظه‌ی رسیدن به سقف متوقف می‌شود، نه بعد از پر شدن کامل دیسک
- محدودسازی پسوند به یک whitelist (`ALLOWED_EXTENSIONS`)
- تابع `_sanitize_filename` برای حذف اجزای مسیر (`../`) و کاراکترهای غیرمجاز از نام فایل
- هماهنگ‌سازی `client_max_body_size` در `nginx.conf` با همان سقف بک‌اند

---

## پشتیبانی چندگانه از LLM (شامل OpenRouter)

پروژه از قبل یک لایه‌ی انتزاعی (`backend/agent/llm.py`) برای چند LLM Provider داشت (Gemini، Anthropic، Groq، OpenAI، Ollama، و حتی OpenRouter) اما `OPENROUTER_API_KEY` در `docker-compose.yml` تعریف نشده بود. این مورد اضافه شد تا بتوان مدل‌های در دسترس روی OpenRouter (شامل مدل‌های آزمایشی/رایگان) را نیز امتحان کرد.

```mermaid
flowchart LR
    ENV[".env<br/>DOCGEN_LLM_PROVIDER"] --> LLM["backend/agent/llm.py<br/>(لایه‌ی انتزاعی)"]
    LLM --> P1[Gemini]
    LLM --> P2[Anthropic]
    LLM --> P3[Groq]
    LLM --> P4[OpenAI]
    LLM --> P5[Ollama - لوکال]
    LLM --> P6["OpenRouter<br/>(دسترسی به مدل‌های متعدد)"]
```

> نکته‌ی عملی که حین تست کشف شد: مدل‌های «reasoning» (مثل `GLM-5.3-flash` که پشت نام مستعار «Ox Alpha» بود) گاهی فیلد `content` را خالی برمی‌گردانند چون خروجی اصلی‌شان در بخش استدلال مصرف می‌شود؛ برای تولید سند نهایی، مدل‌های معمولی (مثل `gpt-4o-mini`) پایدارتر عمل کردند.

---

## فاز ۲.۵ (بخش ۱) — پارسر ساختاری Device DOCX

**مشکلی که حل شد:** تا پیش از این، فایل Device فقط ذخیره می‌شد و محتوای آن هرگز خوانده نمی‌شد — دقیقاً برخلاف Reference DOCX که ساختارش (heading، جدول، تصویر) استخراج می‌شد.

```mermaid
flowchart TD
    subgraph old["قبل از این فاز"]
        D1["Device DOCX آپلود می‌شود"] --> D2["فقط فایل ذخیره می‌شود"]
        D2 --> D3["❌ محتوا هرگز خوانده نمی‌شود"]
    end
    subgraph new["بعد از این فاز"]
        E1["Device DOCX آپلود می‌شود"] --> E2["فایل ذخیره می‌شود"]
        E2 --> E3["پارسر اجرا می‌شود<br/>(همان منطق Reference)"]
        E3 --> E4["heading / پاراگراف / جدول /<br/>رفرنس تصویر استخراج می‌شود"]
        E4 --> E5["در جدول device_document_sections<br/>ذخیره می‌شود"]
    end
```

**اجزای فنی ساخته‌شده:**

| فایل | نقش |
|---|---|
| `models/device_document.py` | مدل جدید `DeviceDocumentSection` (مشابه `DocumentTemplate` ولی متصل به Device) |
| `db/migrations/versions/0007_device_document_sections.py` | Migration جدول جدید |
| `services/device_document_parser.py` | تابع `parse_and_store_sections` — از همان `extract_docx` و `split_sections` موجود (که از قبل عمومی بودند) استفاده می‌کند |
| `api/routes/devices.py` | Endpoint جدید `GET /devices/documents/{document_id}/sections` برای مشاهده‌ی نتیجه |

**محدودیت شناخته‌شده:** این پارسر فقط فایل‌های `.docx` را می‌خواند (چون `python-docx` مستقیماً از XML می‌خواند). فایل‌های `.doc` (قدیمی) و `.pdf` رد می‌شوند — بدون خراب کردن آپلود — و منتظر «خط‌لوله‌ی Vision» (بخش دوم فاز ۲.۵) می‌مانند.

**باگ کشف و رفع‌شده حین تست:** یک عنوان (heading) بلندتر از حد مجاز ستون دیتابیس (۲۵۵ کاراکتر) باعث کرش کل درخواست آپلود می‌شد (`StringDataRightTruncationError`). رفع شد با:
1. کوتاه‌سازی (truncate) مقادیر طولانی پیش از ذخیره
2. جداسازی تراکنش ذخیره‌ی section‌ها از تراکنش اصلی آپلود، تا خطای احتمالی بعدی در این بخش، آپلود فایل را از کار نیندازد

**نتیجه‌ی تست واقعی:** روی یک فایل نمونه‌ی ۱۷ صفحه‌ای (`Input-GUI_VL8.docx`)، ۲۱ section با موفقیت استخراج شد — شامل عنوان‌ها، متن، و جدول‌ها (مثل `Settings Page` و جدول استانداردها).

---

## ساختار برنچ‌ها

```mermaid
gitGraph
    commit id: "master"
    branch phase1_upload
    checkout phase1_upload
    commit id: "فاز ۱: آپلود Device"
    checkout main
    merge phase1_upload
    branch security_fix
    checkout security_fix
    commit id: "سخت‌سازی امنیتی"
    commit id: "docker-compose:<br/>huggingface + OpenRouter"
    branch parser_v2
    checkout parser_v2
    commit id: "فاز ۲.۵: پارسر Device"
    commit id: "رفع باگ طول متن"
    checkout security_fix
    merge parser_v2
```

> نکته: به‌دلیل نام‌گذاری داخلی Mermaid، برنچ اصلی در نمودار بالا `main` نامیده شده؛ در ریپازیتوری واقعی این برنچ `master` نام دارد.
>
> نگاشت نام‌های کوتاه‌شده در نمودار (به‌دلیل محدودیت Mermaid با کاراکتر `/` در اسم برنچ): | نام کوتاه در نمودار | نام واقعی برنچ |
> |---|---|
> | `phase1_upload` | `feature/new-feature-document-update` |
> | `security_fix` | `fix/device-upload-security` |
> | `parser_v2` | `feature/document-parser-v2` |

| برنچ | محتوا | وضعیت |
|---|---|---|
| `feature/new-feature-document-update` | فاز ۱ (آپلود Device) | ✅ merge شده به master |
| `fix/device-upload-security` | سخت‌سازی امنیتی + تنظیمات docker-compose + فاز ۲.۵ (پارسر) | فعلاً روی این برنچ کار می‌شود |

---

## نقشه‌راه باقی‌مانده

```mermaid
flowchart TD
    A["فاز ۲.۵ (بخش ۲)<br/>خط‌لوله‌ی Vision<br/>Surya Layout + Gemini<br/>برای PDF / .doc / تصاویر"] --> B["فاز ۳.۵<br/>chunk + embed کردن<br/>محتوای Device در Knowledge Base"]
    B --> C["فاز ۴<br/>Agent مقایسه‌گر واقعی<br/>Reference در برابر Device"]
    C --> D["فاز ۵<br/>DOCX Builder واقعی<br/>ساخت جدول و درج تصویر واقعی"]
    D --> E["فاز ۶ (آینده، جدا)<br/>تولید تصویر/نمودار کاملاً جدید"]
```

جزئیات کامل هر فاز (شامل ابزارهای قابل‌نصب و مقایسه‌ی مدل‌های Vision) در سند جداگانه‌ی نقشه‌راه پروژه موجود است.

---

*آخرین به‌روزرسانی: این سند به‌صورت دستی هم‌زمان با پیشرفت پروژه به‌روزرسانی می‌شود.*
